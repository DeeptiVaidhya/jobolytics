...
class Jobs extends CI_Controller {
	...
	public function search()
	{
		...
		if($this->input->post('search'))
		{
			...
			if(empty($this->input->post('job_title')) && empty($this->input->post('location')) && empty($this->input->post('category')) && empty($this->input->post('experience')) && empty($this->input->post('job_type')) && empty($this->input->post('employment_type')) )
			{
				$this->session->set_flashdata('message_name', '<span style="color:red;font-size:20px;text-align:center;">Please select check box</span>');
	...
	public function jobs_by_category()
	{
		// $data['categories'] = $this->job_model->get_categories_with_jobs();
			...
			$data['categories'] = $this->job_model->get_categories_with_jobs();
			$data['static_categories'] = $this->job_model->get_static_categories_with_jobs();

		//$categories = $this->job_model->get_categories_with_jobs();

		if (!empty($categories)) {
		}else{
		}
		//$data['static_categories'] = $this->job_model->get_categories_with_jobs();
	...
	{
		$data['location'] = $this->job_model->get_cities_with_jobs();
	...
	public function location($title)
	{
		...
		$search['location'] = $title; // get city id by title
	...
	{	
		...
		$data['cities_job'] = $this->home_model->get_cities_with_jobs();
		...
		$data['jobs'] = $this->home_model->get_jobs(8,0);//right sidebar
